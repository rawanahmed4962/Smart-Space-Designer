// ==========================================
// 0. GLOBAL VARIABLES (بنضبطهم مرة واحدة بس)
// ==========================================
...
console.log("🚀 Script is running! Project ID:", currentProjectId);
...
document.getElementById('add-model-btn').addEventListener('click', () => {
    ...
    gltfLoader.load(modelPath, function (gltf) {
        ...
        scene.add(wrapper);
        loadedModels.push(wrapper); // الإضافة للقائمة هنا عشان السيف
...
if (currentProjectId) {
    ...
    .then(data => {
        if (data && !data.error) {
            globalProjectName = data.project_name || "My Project";
            ...
            if(data.scene_data) {
                ...
                if (sceneData.roomSettings) {
                    ...
                    updateRoom(); // عشان تبني الحيطان الجديدة
                    ...
                    // تحديث واجهة المستخدم (الـ Sliders)
                ...
                if (sceneData.furniture && sceneData.furniture.length > 0) {
                    sceneData.furniture.forEach(item => {
                        gltfLoader.load(item.modelPath, (gltf) => {
                            ...
                            wrapper.userData = { path: item.modelPath }; // مهم عشان السيف
                            ...
                            scene.add(wrapper);
                            loadedModels.push(wrapper); // التكة المهمة عشان العفش يتسيف تاني
                        });
                    });
                }
