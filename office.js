...
console.log("🚀 Office Script is running! Project ID:", currentProjectId);
...
const ceilingLight = new THREE.SpotLight(0xffffff, 2.0); 
        ...
        wrapper.traverse((node) => {
            if (node.isMesh) { 
                ...
                if (node.material) {
                    ...
                    if (node.material.metalness === 1 && node.material.roughness === 0 && !node.material.envMap) {
...
if (currentProjectId) {
    ...
    .then(data => {
        if (data && !data.error) {
            globalProjectName = data.project_name || "My Office Project";
            ...
            if(data.scene_data) {
                ...
                if (sceneData.roomSettings) {
                    ...
                    wallMaterial.color.set(sceneData.roomSettings.wallColor || 0x2c3e50);
                    floorMaterial.color.set(sceneData.roomSettings.floorColor || 0x5d4037);
                    ...
                    if(document.getElementById('wall-color')) document.getElementById('wall-color').value = sceneData.roomSettings.wallColor || "#2c3e50";
                    if(document.getElementById('floor-color')) document.getElementById('floor-color').value = sceneData.roomSettings.floorColor || "#5d4037";
                }
                        ...
                        gltfLoader.load(item.modelPath, (gltf) => {
                            ...
                            wrapper.traverse((node) => {
                                if (node.isMesh) { 
                                    node.castShadow = true; 
                                    node.receiveShadow = true;
                                    if (node.material && node.material.metalness === 1 && node.material.roughness === 0 && !node.material.envMap) {
                                        node.material.metalness = 0.2;
                                        node.material.roughness = 0.8;
                                    }
                                }
...
/// ==========================================
// 11. EXIT & SAVE LOGIC (Bulletproof Version)
// ==========================================
...
// --- 2. برمجة زرار الـ Save (الطريقة اللي مستحيل تبوظ) ---
document.addEventListener('click', (e) => {
    const saveBtnClicked = e.target.closest('#save-project-btn');
    if (saveBtnClicked) {
        e.preventDefault();
...
// --- 3. قفل الـ Modal بتاع المشروع الجديد ---
document.addEventListener('click', (e) => {
    const cancelBtn = e.target.closest('#cancel-save-btn');
    if(cancelBtn) {
        const sModal = document.getElementById('save-modal');
        if(sModal) sModal.style.display = 'none';
        window.isExitingAfterSave = false;
    }
});

// --- 4. تأكيد السيف من جوه الـ Modal ---
document.addEventListener('click', (e) => {
    const confirmBtn = e.target.closest('#confirm-save-btn');
    if(confirmBtn) {
...
function triggerSaveWorkflow() {
    ...
    if (currentProjectId && currentProjectId !== "") {
        ...
        if(sModal) {
            ...
            console.error("❌ Save Modal not found in HTML!");
        }
    }
}

// --- 6. وظيفة الإرسال للـ PHP ---
function executeFinalSave(projName, projDesc) {
    ...
    let detectRoomType = "living_room";
